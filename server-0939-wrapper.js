...
html=applyPatch(html,'patch-0935-from-0934.json','v0.9.35');
if(Buffer.byteLength(html)!==291848||sha(html)!=='5540c60581d1e55a061062e5b6e6a9edb731d1f64ebdb46343c6458df36f9e4d')throw Error('v0.9.35 HTML integrity mismatch');

html=applyPatch(html,'patch-0936-from-0935.json','v0.9.36');
if(Buffer.byteLength(html)!==292754||sha(html)!=='8c4889f5d82f49dfbaae509c17e73794cc88310ff4027e51b0139411f58b18cc')throw Error('v0.9.36 HTML integrity mismatch');

html=applyPatch(html,'patch-0937-from-0936.json','v0.9.37');
if(Buffer.byteLength(html)!==294283||sha(html)!=='20d2b997d652730219bc2f2cb57720a643336d262cf9edcc57cb23bbace6d6b7')throw Error('v0.9.37 HTML integrity mismatch');

html=applyPatch(html,'patch-0938-from-0937.json','v0.9.38');
if(Buffer.byteLength(html)!==294733||sha(html)!=='e328f47e215baa9403ddbbf8546a090c8de10bbd04d7b66511464f3f10a27250')throw Error('v0.9.38 HTML integrity mismatch');

html=applyPatch(html,'patch-0939-from-0938.json','v0.9.39');
if(Buffer.byteLength(html)!==299645||sha(html)!=='5c094aa7010452f7f27a529d1a81071597a924978da00cfba0d96d426a5ba575')throw Error('v0.9.39 HTML integrity mismatch');
...
source=source.replace("version:'0.9.23'","version:'0.9.39'");
source=source.replace('JEWEL de PON v0.9.23 server:','JEWEL de PON v0.9.39 server:');
...
source=source.replace('const SR=16000',"for(const name of [\"combo1\",\"combo2\",\"combo3\",\"combo4\",\"combo5\",\"combo6\",\"combo7\",\"combo8\",\"combo9\",\"combo10\",\"item\",\"ojamaochiru\",\"katta\",\"ugokasu\",\"maketa\",\"seriagari\",\"danger\",\"ojamahenkan\"])CUSTOM_BGM[name]=fs.readFileSync(path.join(ROOT,'audio',name+'.mp3'));\nconst SR=16000");
...
source=source.replace('const SR=16000',"for(const name of [\"gameover\",\"banmen\",\"321\",\"go\"])CUSTOM_BGM[name]=fs.readFileSync(path.join(ROOT,'audio',name+'.mp3'));\nconst SR=16000");
const runtimeFile=path.join(ROOT,'server-0939-runtime.js');
