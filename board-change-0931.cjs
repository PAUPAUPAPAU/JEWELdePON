...
function sanitizeBoard(s){
  ...
  const panel=p=>{if(p===null)return null;if(!p||!Number.isInteger(p.c)||p.c<1||p.c>7)throw Error('BAD_CHANGE_BOARD');return {c:p.c,special:specials.has(p.special)?p.special:null};};
