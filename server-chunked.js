...
function loadChunkedGzipBase64(prefix){
  ...
  const names=fs.readdirSync(dir).filter(n=>n.startsWith(prefix+'-')&&n.endsWith('.b64')).sort();
...
const AUDIO_STORE = JSON.parse(loadChunkedGzipBase64('audio').toString('utf8'));
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='GET' && u.pathname.startsWith('/audio/') && u.pathname.endsWith('.mp3')){
      ...
      res.writeHead(200,{'Content-Type':'audio/mpeg','Content-Length':buf.length,'Cache-Control':'public, max-age=31536000, immutable','X-Content-Type-Options':'nosniff'});
      res.end(buf);
      return;
    }
    if(req.method==='GET' && u.pathname==='/health'){
      return json(res,200,{ok:true,service:'jewel-de-pon',version:'0.9.22',rooms:rooms.size,uptime:Math.floor(process.uptime())});
    }
    ...
    if(req.method==='POST' && u.pathname==='/api/start'){
      ...
      return json(res,200,{room:snapshot(room),matchId:room.matchId,startAt:room.startAt});
    }
    ...
    if(req.method==='POST' && u.pathname==='/api/ko'){
      ...
      return json(res,200,{ok:true,winnerId:room.winnerId,loserId:room.loserId});
    }
